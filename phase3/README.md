...
| P1b-12a height boundary fast path probe | [`results/metrics/p1b_aggregation_attention_plugin_height_fastpath_engine_benchmark_summary.md`](results/metrics/p1b_aggregation_attention_plugin_height_fastpath_engine_benchmark_summary.md) | 高度方向 fast path correctness 通过，但 p50 `52.510ms`，慢于 P1b-7 的 `52.311ms`，因此不采纳 |
| P1b-13a shared-center qkv copy probe | [`results/metrics/p1b_aggregation_attention_plugin_shared_center_copy_engine_benchmark_summary.md`](results/metrics/p1b_aggregation_attention_plugin_shared_center_copy_engine_benchmark_summary.md) | 复用 shared tile center 替代 qkv global copy correctness 通过，但 p50 `52.889ms`，明显慢于 P1b-7，因此不采纳 |
| P1b-14a restrict/ldg read-only hint probe | [`results/metrics/p1b_aggregation_attention_plugin_restrict_ldg_engine_benchmark_summary.md`](results/metrics/p1b_aggregation_attention_plugin_restrict_ldg_engine_benchmark_summary.md) | 给输入指针加 `__restrict__` 并把只读 global load 改为 `__ldg()` 后 correctness 通过，但 p50 `53.356ms`，明显慢于 P1b-7，因此不采纳 |
| P1b-15a half-warp-per-pixel probe | [`results/metrics/p1b_aggregation_attention_plugin_p1b15a_engine_benchmark_summary.md`](results/metrics/p1b_aggregation_attention_plugin_p1b15a_engine_benchmark_summary.md) | 半个 warp 分摊一个像素的 16 通道计算 correctness 通过，但 p50 `67.613ms`，严重慢于 baseline 和 P1b-7，因此不采纳 |
...
- [x] Step 8.26：评估 P1b-12a height-boundary fast path probe。产物：[`results/metrics/p1b_aggregation_attention_plugin_height_fastpath_engine_benchmark_summary.md`](results/metrics/p1b_aggregation_attention_plugin_height_fastpath_engine_benchmark_summary.md)。结论：中间 spatial block 跳过高度方向 halo 边界判断后 correctness 通过，但 p50 为 `52.510ms`，慢于 P1b-7 的 `52.311ms`；减少边界判断不是当前主瓶颈，因此不采纳，主线恢复 P1b-7。
