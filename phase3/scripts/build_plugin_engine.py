"""Build a real EfficientViT TensorRT engine from the Plugin-patched ONNX graph.

This Step 6 script validates parser/build integration only. Correctness and
latency comparison against the Phase 2 TensorRT baseline are Step 7 tasks.
"""
...
SCRIPT_NAME = "build_plugin_engine.py"
PLUGIN_NAME = "EdgesegReluLinearAttention_TRT"
PLUGIN_VERSION = "1"
PLUGIN_NAMESPACE = "edgeseg"
DEFAULT_ONNX = Path("phase3/results/onnx/efficientvit_seg_b0_cityscapes_1024x2048_relu_linear_att_plugin.onnx")
DEFAULT_ENGINE = Path("phase3/results/engines/efficientvit_seg_b0_cityscapes_1024x2048_relu_linear_att_plugin_fp32.engine")
DEFAULT_METADATA = Path("phase3/results/metrics/relu_linear_attention_plugin_engine_build.json")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build an EfficientViT TensorRT engine from Plugin-patched ONNX.")
    p.add_argument("--onnx", type=Path, default=DEFAULT_ONNX, help="Plugin-patched ONNX path.")
    p.add_argument("--engine", type=Path, default=DEFAULT_ENGINE, help="Output Plugin TensorRT engine path.")
...
def build_engine(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if not onnx_path.is_file():
        raise FileNotFoundError(f"Plugin ONNX file not found: {onnx_path}")
    ...
    if serialized is None:
        raise RuntimeError("TensorRT builder returned None serialized Plugin engine")
    ...
    metadata = {
        ...
        "notes": [
            "This validates Plugin-patched ONNX parser/build integration only.",
            "Step 7 must validate end-to-end correctness and latency.",
...
def main() -> None:
    ...
    print(
        "Plugin TensorRT engine build complete: "
