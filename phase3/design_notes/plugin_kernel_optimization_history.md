...
1. **扩大边界是有价值的**。P1b-5 把 `aggregation + attention_core` proxy 从 Phase 2 baseline 的 `5.443 ms / 38 launches` 降到 `3.241 ms / 6 launches`。
2. **P1b-5 已接近 P1a proxy 水平**。P1a-3b 的 `aggregation + plugin` proxy 约 `3.062 ms / 30 launches`；P1b-5 为 `3.241 ms / 6 launches`，kernel time 仍略高，但 launch 数显著更少，端到端 p50 也达到 `52.455 ms`。
3. **下一步若继续 P1b，应继续盯住 `fusedAggregationCatKernel`**。P1b-5 后它仍约 `1.926 ms/iter`，占 P1b Plugin layer 约 `59.44%`。
...
| P1b-6 | 继续优化 `fusedAggregationCatKernel` | 降低当前 P1b 内部主瓶颈 | P1b-5 后仍约 `1.926 ms/iter`；可考虑 `kDepthwiseTileChannels=16`、warp-level output reuse、CTA layout A/B，但收益递减且风险更高 |
