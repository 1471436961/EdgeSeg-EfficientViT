...
> 目的：验证 `stage2=P1b-7 aggregation+cat+relu_linear_att` 与 `stage3=P1a-3b relu_linear_att-only` 的组合是否优于当前最强参照 `P1a stage2+stage3`。
...
如果 P1mix 只优于旧的 stage2-only P1a/P1b，但不优于 `P1a stage2+stage3`，就不应作为新主线。
...
- 如果 P1mix 不优于 `50.838 ms`，当前 Phase 3 MVP 应继续保留为 `P1a stage2+stage3`。
...
2. P1mix 没有稳定优于 `P1a stage2+stage3`。plugin-only p50 只快 `0.095 ms`，低于当前 MX250 的可信差异阈值；Nsight execute kernel avg 反而略慢 `0.104 ms`。
3. P1mix 减少了 launch 数（`163 -> 146`），但 P1b stage2 Plugin layer 边界更重，selected context total 从 `6.436 ms` 增到 `6.624 ms`。
4. 因此当前主线仍应保留为 `P1a stage2+stage3`。P1mix 记录为 `evaluated, not adopted as mainline`，除非后续能显著优化 P1b stage2 kernel。
